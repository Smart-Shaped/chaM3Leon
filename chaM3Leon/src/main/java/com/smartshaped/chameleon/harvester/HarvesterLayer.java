...
/**
 * The HarvesterLayer class is responsible for starting the harvesting process.
 * It reads all the requests from the database, filters the harvesters that
 * match the request, and calls the {@link Harvester#execute(Request)} method
 * for each of them.
 */
public class HarvesterLayer {
     ...
     * <p>
     * The method reads all the requests from the database, filters the harvesters
     * that match the request, and calls the {@link Harvester#execute(Request)}
     * method for each of them. If an exception is thrown during the harvesting
     * process, the request is marked as error and the exception is re-thrown.
     *
        ...
        for (Request request : requests) {
            ...
            try {
                ...
                for (Harvester harvester : filteredHarvesters) {
                    logger.info("Using harvester: {}", harvester.getClass());
     ...
     * <p>
     * It splits the harvester IDs from the request into a set and loops through
     * each harvester, checking if its ID is present in the set. If a match is found,
     * the harvester is added to the resulting list.
     *
    ...
    private static List<Harvester> filterHarvesters(List<Harvester> harvesters, Request request) {
        ...
        logger.info("Number of harvesters: {}", harvesters.size());
        ...
        Set<String> set = Arrays.stream(request.getHarvesterIds().split(",")).collect(Collectors.toSet());
