...
/**
 * Utility class that extends {@link ConfigurationUtils} for reading configuration files related to Harvester layer.
 */
public class HarvesterConfigurationUtils extends ConfigurationUtils {
    ...
    /**
     * Gets the instance of the HarvesterConfigurationUtils class, which is responsible for loading the configurations for the Harvester layer.
     * <p>
     * If the configuration is not yet loaded, it will be loaded and stored in the static variable.
     * <p>
     *
    ...
    public static HarvesterConfigurationUtils getHarvesterConf() throws ConfigurationException {
        ...
        if (configuration == null) {
            logger.info("No previous harvester configuration found, loading new configurations.");
     ...
     * <p>
     * This method iterates over the list of harvesters in the configuration and returns the harvester ID for the class
    ...
    public String getHarvesterId(String className) throws ConfigurationException {
        logger.info("Retrieving harvester ID for class: {}", className);
        List<HierarchicalConfiguration<ImmutableNode>> harvesterList = config.childConfigurationsAt(HARVESTERS);
    ...
    /**
     * Returns a list of instances of the Harvester class, based on the harvesters listed in the configuration.
     * <p>
     * The method iterates over the list of harvesters in the configuration and instantiates each one using the
     * {@link #loadInstanceOf(String, Class)} method. The instantiated harvesters are then returned as a list.
     * <p>
     *
     * @return the list of harvesters
     * @throws ConfigurationException if no harvesters are defined in the configuration or if there is an error while
     *                                instantiating a harvester
     */
        ...
        while (keys.hasNext()) {
            ...
            if (segments.length == 4 && "harvesters".equals(segments[1]) && "class".equals(segments[3])) {
                ...
                if (readerClassName == null || readerClassName.trim().isEmpty()) {
                    throw new ConfigurationException("At least a harvester must be defined in harvester.harvesters config");
                }
                try {
                    harvesters.add(loadInstanceOf(readerClassName, Harvester.class));
                } catch (ConfigurationException e) {
                    throw new ConfigurationException(COULD_NOT_INSTANTIATE + HdfsReader.class + DUE_TO_EXCEPTION, e);
                }
            }
        }
     ...
     * <p>
     * The method attempts to fetch the RequestHandler class name from the configuration,
     * and if it is set to "default", it returns a new instance of the default RequestHandler.
     * Otherwise, it tries to instantiate the specified RequestHandler class.
     * <p>
     *
     * @return an instance of RequestHandler
     * @throws ConfigurationException if no RequestHandler is defined in the configurations
     *                                or if there is an error during instantiation
    ...
    public RequestHandler getRequestHandler() throws ConfigurationException, CassandraException {
        ...
        logger.info("Loading RequestHandler: {}", handlerName);

        if (handlerName == null) {
            throw new ConfigurationException("No RequestHandler defined in configurations in harvester.RequestHandler");
        }
        ...
        try {
            ...
            throw new ConfigurationException(COULD_NOT_INSTANTIATE + RequestHandler.class + DUE_TO_EXCEPTION, e);
        }
     ...
     * <p>
     * This method constructs the base path for the preprocessor parameters
     * using the provided class name and iteratively fetches each key-value
     * pair from the configuration. If a key contains a period, only the
     * segment after the last period is used as the final key in the map.
     * <p>
     *
    ...
    public Map<String, String> getPreprocessorParams(String className) throws ConfigurationException {
        logger.info("Starting to retrieve preprocessor parameters for class: " + className);

        Map<String, String> params = new HashMap<>();
        String basePath = confRoot + PREPROCESSOR + SEPARATOR + className + SEPARATOR + PARAMS;
        logger.info("Base path for preprocessor keys: " + basePath);

        Iterator<String> iterator = config.getKeys(basePath);
        if (!iterator.hasNext()) {
            logger.info("No keys found under base path: " + basePath);
        }
        ...
        while (iterator.hasNext()) {
            ...
            try {
                ...
                logger.info("Retrieved value for key '" + originalKey + "': " + value);
        ...
        logger.info("Finished retrieving preprocesssor parameters for class: " + className + ". Total parameters: " + params.size());
        return params;
    ...
    /**
     * Retrieves a preprocessor instance based on the preprocessor name configured for the provided harvester ID.
     * <p>
     * If no preprocessor is specified in the configurations for the given harvester ID, an empty preprocessor is used.
     * <p>
     *
    ...
    public Preprocessor getPreprocessor(String harvesterId) throws ConfigurationException {
        ...
        if (preprocessorName.trim().isEmpty()) {
            logger.info("No preprocessor specified in the configurations, using empty one...");
     ...
     * <p>
     * The method constructs the base path for the url parameters
     * using the provided class name and iteratively fetches each key-value
     * pair from the configuration. If a key contains a period, only the
     * segment after the last period is used as the final key in the map.
     * <p>
    ...
    public Map<String, String> getUrlParams(String className) throws ConfigurationException {
        logger.info("Starting to retrieve url parameters for class: " + className);

        String basePath = confRoot + DOWNLOADER + SEPARATOR + className + SEPARATOR + URL;
        logger.info("Base path for configuration keys: " + basePath);
     ...
     * <p>
     * The method constructs the base path for the query parameters
     * using the provided class name and iteratively fetches each key-value
     * pair from the configuration. If a key contains a period, only the
     * segment after the last period is used as the final key in the map.
     * <p>
    ...
    public Map<String, String> getQueryParam(String className) throws ConfigurationException {
        logger.info("Starting to retrieve query parameters for class: " + className);
     ...
     * <p>
     * This method iterates over the configuration keys under the given base path and
     * constructs a map where each key is the segment after the last period in the
     * original key, and the value is the corresponding configuration value.
     * <p>
     *
    ...
    private Map<String, String> getParams(String basePath) throws ConfigurationException {
        ...
        logger.info("Finished retrieving parameters, total parameters: " + params.size());
        return params;
    }

    /**
     * Retrieves a transformer instance based on the transformer name configured for the provided harvester ID.
     * <p>
     * If no transformer is specified in the configurations for the given harvester ID, null is returned.
     * <p>
     *
     * @param harvesterId the ID of the harvester for which the transformer is being retrieved.
     * @return an instance of the transformer class configured for the given harvester ID, or null if none is specified.
     * @throws ConfigurationException if there is an error while loading the transformer instance.
     */
    public DatasetTransformer<Dataset<Row>> getTransformer(String harvesterId) throws ConfigurationException {
    ...
    /**
     * Retrieves a Downloader instance based on the downloader name configured for the given harvester ID.
     * <p>
     * This method fetches the downloader class name from the configuration using the provided harvester ID.
     * If no class name is specified, it throws a ConfigurationException. Otherwise, it attempts to load and
     ...
     * @throws ConfigurationException if no downloader is specified in the configurations or if there is an error during instantiation.
     */
    public Downloader getDownloader(String harvesterId) throws ConfigurationException {
        logger.info("Attempting to retrieve downloader name for harvester ID: {}", harvesterId);
        String downloaderName = config.getString(harvesterId + SEPARATOR.concat(DOWNLOADER.concat(SEPARATOR.concat(CLASS))));
        ...
        try {
            logger.info("Loading instance of downloader: {}", downloaderName);
            return loadInstanceOf(downloaderName, Downloader.class);
        } catch (ConfigurationException e) {
            throw new ConfigurationException(COULD_NOT_INSTANTIATE + Downloader.class + DUE_TO_EXCEPTION, e);
        }
    }
     ...
     * <p>
     * This method fetches the output path from the configuration using the provided harvester ID.
    ...
    public String getOutputPath(String harvesterId) {
        return config.getString(harvesterId + SEPARATOR.concat(SAVER.concat(SEPARATOR.concat(PATH))), "");
    }

    /**
     * Retrieves the input path configured for the given harvester ID.
     * <p>
     * This method fetches the input path from the configuration using the provided harvester ID.
