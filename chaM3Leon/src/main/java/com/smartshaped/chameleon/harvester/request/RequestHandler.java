...
public class RequestHandler {
     ...
     * <p>
     * The method executes a select query on the "request" table and retrieves all the rows with state set to 'false'.
     * It then loops through the rows and populates a list of {@link Request} objects. The objects are populated with the
     * values from the row and then validated with a {@link RequestValidator}. If the request is not valid, it is
     * discarded. The method then returns an array of valid requests.
     *
     ...
     * <p>
     * This method constructs an update query to change the state of the request
     * identified by its UUID in the database. The new state is provided as a parameter.
     *
     ...
     * <p>
     * This method will close the underlying connection to the Cassandra database.
     * The method is idempotent, so it is safe to call it multiple times.
     */
